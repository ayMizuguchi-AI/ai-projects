...
# ----------検証用----------
    ...
    V_train, V_val, labels_train, labels_val = train_test_split(
        V, labels, test_size=0.2, random_state=42, stratify=labels
    )
# --------------------------
    ...
    print("\nAIの学習を開始します...")
    # ----------検証用----------
    model.fit(V_train, labels_train)
    # --------------------------
    print("学習完了")
...
# ----------検証用----------
    y_val_pred = model.predict(V_val)

    # 的中した人数(True Positives)を計算
    tp = sum((labels_val[i] == 1 and y_val_pred[i] == 1) for i in range(len(labels_val)))
    # 見逃した人数(False Negatives)を計算
    fn = sum((labels_val[i] == 1 and y_val_pred[i] == 0) for i in range(len(labels_val)))
    # 冤罪(False Positives)を計算
    fp = sum((labels_val[i] == 0 and y_val_pred[i] == 1) for i in range(len(labels_val)))

    # サンプル数
    true_positive_total = sum(labels_val)
# --------------------------
    ...
    # スコア計算
# ----------検証用----------
    rec = recall_score(labels_val, y_val_pred, zero_division=0)
    prec = precision_score(labels_val, y_val_pred, zero_division=0)
# --------------------------
...
# -----追加-----
            # タイプ判定 デフォルト値  
            predicted_type = "suspicious_order"

            # 特徴量取得
            val_price_mult  = user_price_multiplier[target_id]
            val_shared_ip   = user_shared_ip_max[target_id]
            val_max_qty     = user_max_qty[target_id]
            val_max_z       = user_max_zscores[target_id]

            # タイプ割り振り
            if val_price_mult > 1.05:
                predicted_type = "price_mismatch"
            elif val_shared_ip >= 3:
                predicted_type = "unusual_access"
            elif val_max_qty >= 5:
                predicted_type = "rapid_orders"
            elif val_max_z >= 3.0:
                predicted_type = "suspicious_order"
# --------------
            # 二重登録防止
            if not any(a["entity_id"] == target_id for a in result):
                result.append({
                    "anomaly_id": f"A{len(result)+1:03d}",    
# -----追加-----
                    "type": predicted_type, #AIが見つけた怪しい動き        
# --------------
                    "entity_id": target_id,    
# -----追加-----
...
# --------------
                    ...
                    "details": {
                        ...
                        "value": round(float(anomaly_score), 2),
                        "threshold": 0.50,
                        "deviation_factor": round(float(anomaly_score / 0.5), 2)
                    }
                })
